...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_decorations(self, page_count):
        ...
        if self._pageNumber > 1:
            self.drawString(54, 750, "RAILBLOCK — AI, ML & OPTIMIZATION FORENSIC CODE AUDIT")
            self.setFont("Helvetica", 8)
            self.setFillColor(colors.HexColor("#718096"))
            self.drawRightString(612 - 54, 750, "SIH GRAND FINALE TECHNICAL EVALUATION")
        ...
        self.setFont("Helvetica", 8)
        self.setFillColor(colors.HexColor("#718096"))
        self.drawString(54, 36, "CONFIDENTIAL — FOR JURY EVALUATION ONLY | SOURCE-CODE GROUND TRUTH")
...
def build_pdf():
    md_path = "/Users/kakulrathi/SIH-FINAL/railway_maintainance_system/AI_ML_OPTIMIZATION_COMPLETE_AUDIT.md"
    pdf_path = "/Users/kakulrathi/SIH-FINAL/railway_maintainance_system/AI_ML_OPTIMIZATION_COMPLETE_AUDIT.pdf"
    ...
    while i < len(lines):
        ...
        if stripped.startswith("# RAILBLOCK:"):
            ...
            story.append(Paragraph("Ground-Truth Source Code Verification & Forensic Technical Audit", subtitle_style))
            story.append(HRFlowable(width="100%", thickness=1.5, color=colors.HexColor('#0284C7'), spaceBefore=2, spaceAfter=8))
            i += 1
            continue

        if stripped.startswith("# PART") or stripped.startswith("# Executive"):
        ...
        if re.match(r"^\d+\.\s", stripped):
            ...
            formatted = re.sub(r"\*\*(.*?)\*\*", r"<b>\1</b>", formatted)
            story.append(Paragraph(f"{stripped[:2]} {formatted}", bullet_style))
            i += 1
            continue
